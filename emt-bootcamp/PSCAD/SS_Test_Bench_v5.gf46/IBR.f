...
! Control Signals
      ...
      REAL     RT_10, F_POI, DBlk, Igrid_POC(3)
      REAL     Vgrid_POC(3)
...
! Array (1:3) quantities...
      DO IT_0 = 1,3
         Igrid_POC(IT_0) = STOF(ISTOF + 23 + IT_0)
         Vgrid_POC(IT_0) = STOF(ISTOF + 26 + IT_0)
      END DO
...
! 40:[var] Variable Input Slider 'Vref'
      RT_6 = CX(CXMAP(ICX+2))

! 50:[var] Variable Input Slider 'Pref'
      RT_7 = CX(CXMAP(ICX+3))

! 60:[ratelimit] Rate Limiter 
!  C/s
      CALL RATE_LIMITER(100.0,100.0,RT_7,RT_5)
...
! 100:[var] Variable Input Slider 'PPC_Kp_P'
      PPC_Kp_P = CX(CXMAP(ICX+4))

! 110:[var] Variable Input Slider 'PPC_Kp_V'
      PPC_Kp_V = CX(CXMAP(ICX+5))

! 120:[var] Variable Input Slider 'PPC_Kp_PF'
      PPC_Kp_PF = CX(CXMAP(ICX+6))

! 130:[var] Variable Input Slider 'PPC_Kp_Q'
      PPC_Kp_Q = CX(CXMAP(ICX+7))

! 140:[var] Variable Input Slider 'PPC_Ti_P'
      PPC_Ti_P = CX(CXMAP(ICX+8))

! 150:[var] Variable Input Slider 'PPC_Ti_V'
      PPC_Ti_V = CX(CXMAP(ICX+9))

! 160:[var] Variable Input Slider 'PPC_Ti_PF'
      PPC_Ti_PF = CX(CXMAP(ICX+10))

! 170:[var] Variable Input Slider 'PPC_Ti_Q'
      PPC_Ti_Q = CX(CXMAP(ICX+11))
...
! 200:[pgb] Output Channel 'Ppu'

      PGB(IPGB+2) = RT_1

! 210:[pgb] Output Channel 'Qpu'
...
! Array (1:3) quantities...
      DO IT_0 = 1,3
         STOF(ISTOF + 23 + IT_0) = Igrid_POC(IT_0)
         STOF(ISTOF + 26 + IT_0) = Vgrid_POC(IT_0)
      END DO
